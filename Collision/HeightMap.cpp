	...
	for (int l = 0; l < m_HeightMapLength; ++l)
	{
		for (int w = 0; w < m_HeightMapWidth; ++w)
		{
			if (w < m_HeightMapWidth - 1 && l < m_HeightMapLength - 1)
			{
				...
				XMVECTOR center = (v0 + v1 + v2) / 3;
				XMStoreFloat3(&m_pFaceData[faceIndex + 0].m_vCenter, center);
				...
				center = (v2 + v1 + v3) / 3;
				XMStoreFloat3(&m_pFaceData[faceIndex + 1].m_vCenter, center);
...
bool HeightMap::TestSphereTriangle(Sphere & s, XMVECTOR & a, XMVECTOR & b, XMVECTOR & c, float& penetration, XMVECTOR colNormN)
{
	XMVECTOR spherePos = XMLoadFloat3(&s.mSpherePos);

	// Find point P on triangle ABC closest to sphere center
	XMVECTOR p = ClosestPtPointTriangle(spherePos, a, b, c);

	// Sphere and triangle intersect if the (squared) distance from sphere
	// center to point p is less than the (squared) sphere radius
	XMVECTOR v = p - spherePos;
	float vDot = XMVectorGetX(XMVector3Dot(v, v));
	if (vDot <= s.mSphereRadius * s.mSphereRadius)
	{
		penetration = s.mSphereRadius - sqrt(vDot);
		PositionalCorrection(&s, penetration, colNormN);
		return true;
	}
	return false;
}

void HeightMap::PositionalCorrection(Sphere* sphere, float penetration, XMVECTOR collNorm)
{
	const float percent = 0.99f;
	const float slop = 0.001f;


	XMVECTOR correction = max(penetration - slop, 0.0f) * percent * collNorm;

	XMVECTOR spherePos = XMLoadFloat3(&sphere->mSpherePos);
	spherePos = spherePos + correction;
	
	XMStoreFloat3(&sphere->mSpherePos, spherePos);


}

XMFLOAT3 HeightMap::GetHeightMapFacePos(int faceIndex, int VertIndex)
{
	FaceCollisionData colData = m_pFaceData[faceIndex];

	XMFLOAT3 facePos = XMFLOAT3(0,0,0);

	switch(VertIndex)
	{
	case 0:
		facePos = colData.m_vCenter;
		break;
	case 1:
		facePos = colData.m_v0;
		break;
	case 2:
		facePos = colData.m_v1;
		break;
	case 3:
		facePos = colData.m_v2;
		break;
	}
...
void HeightMap::GetHeightMapVertices(int faceIndex, XMFLOAT3 vertices[FACE_VERTICES_COUNT]) const
{

	vertices[0] = m_pFaceData[faceIndex].m_v0;
	vertices[1] = m_pFaceData[faceIndex].m_v1;
	vertices[2] = m_pFaceData[faceIndex].m_v2;
	vertices[3] = m_pFaceData[faceIndex].m_vCenter;

}

XMVECTOR HeightMap::ClosestPtPointTriangle(XMVECTOR & p, XMVECTOR & a, XMVECTOR & b, XMVECTOR & c)
{
...
bool HeightMap::SphereCollision(XMVECTOR& colNormN, Sphere& mSphere)
{
	...
	for (int f = 0; f < m_HeightMapFaceCount; ++f)
	{
		XMVECTOR vert0 = XMLoadFloat3(&m_pFaceData[f].m_v0);
		XMVECTOR vert1 = XMLoadFloat3(&m_pFaceData[f].m_v1);
		XMVECTOR vert2 = XMLoadFloat3(&m_pFaceData[f].m_v2);
		float penetration;
		colNormN = XMLoadFloat3(&m_pFaceData[f].m_vNormal);
		//012 213
		if (!m_pFaceData[f].m_bDisabled && TestSphereTriangle(mSphere, vert0, vert1, vert2, penetration, colNormN))
		{
			// Needs to be >=0 
			//dprintf("%f\n", penetration);
			dprintf("%i\n", f);
...
bool HeightMap::RayTriangle(int nFaceIndex, const XMVECTOR& rayPos, const XMVECTOR& rayDir, XMVECTOR& colPos, XMVECTOR& colNormN, float& colDist)
 {
	...
	colNormN = XMLoadFloat3(&m_pFaceData[nFaceIndex].m_vNormal);
	 ...
	 XMVECTOR vD = -XMVector3Dot(colNormN, vert0);
	 ...
	 XMVECTOR vDenominator = XMVector3Dot(colNormN, XMVector3Normalize(rayDir));

	 float denom;
	 XMStoreFloat(&denom, vDenominator);
	...
	float numer;
	XMStoreFloat(&numer, XMVector3Dot(colNormN, rayPos));
	numer = (D + numer);
	 //XMVECTOR vNumerator = 
	 
	// XMStoreFloat(&numer, vNumerator);
	 // ...
	 ...
	 //XMFLOAT4 ray;
	 //XMStoreFloat4(&ray, rayPos);

	 colPos = colPos - rayDir;
	 // ...
...
bool HeightMap::PointPlane(const XMVECTOR& vert0, const XMVECTOR& vert1, const XMVECTOR& vert2, const XMVECTOR& pointPos)
 {
	 ...
	 sNormN = XMVector3Cross(sVec0, sVec1);
	 XMVector3Normalize(sNormN);
	 // ...
	
	 // Step 2: Calculate D
	 XMVECTOR D = -XMVector3Dot(sNormN, vert0);
	 XMStoreFloat(&sD, D);

	 // ...
	 
	 // Step 3: Calculate full equation
	 XMVECTOR test = XMVector3Dot(sNormN, pointPos) + D;
	
	 XMStoreFloat(&sNumer, test);
	 // ...
