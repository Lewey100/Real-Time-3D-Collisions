...
public:
	...
	bool SphereCollision(XMVECTOR& colNormN, Sphere& mSphere);
	void PositionalCorrection(Sphere* sphere, float penetration, XMVECTOR colNorm);
	XMFLOAT3 GetHeightMapFacePos(int faceIndex, int VertIndex);
	void GetHeightMapVertices(int faceIndex, XMFLOAT3 vertices[FACE_VERTICES_COUNT]) const;
	int FaceCount() { return m_HeightMapFaceCount; }
    int VertexCount() { return m_HeightMapVtxCount; }
	int DisableBelowLevel(float fYLevel);
	int EnableAll(void);
private:
	...
	bool TestSphereTriangle(Sphere& s, XMVECTOR& a, XMVECTOR& b, XMVECTOR& c, float& penetration, XMVECTOR colNormN);
	XMVECTOR ClosestPtPointTriangle(XMVECTOR& p, XMVECTOR& a, XMVECTOR& b, XMVECTOR& c);
